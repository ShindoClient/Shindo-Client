...
public class GuiBetterResourcePacks extends GuiScreenResourcePacks {
    ...
    public GuiBetterResourcePacks(GuiScreen parentScreen){
        super(parentScreen);
    ...
    protected void actionPerformed(GuiButton button){
        if (button.id == 20){
            ...
            FileUtils.openFolderAtPath(mc.getResourcePackRepository().getDirResourcepacks());
        }
        else if (button.id == 1){
            if (requiresReload){
    ...
    private List<ResourcePackListEntryFound> createAvailablePackList(ResourcePackRepository repository) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                if (mcmeta.isFile()) {
                    // Pasta com pack.mcmeta = resource pack
                    ...
                    // Pasta comum = mostrar como pasta navegável
        ...
        // Remover packs que já estão selecionados
        List<Entry> selectedEntries = repository.getRepositoryEntries();
        list.removeIf(e -> {
            Entry ent = e.func_148318_i();
            return ent != null && selectedEntries.contains(ent);
        });
